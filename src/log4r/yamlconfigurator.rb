...
# == Other Info
#
# Version: $Id: yamlconfigurator.rb,v 1.1.1.1 2004/03/19 03:31:08 fando Exp $
...
module Log4r
  ...
  class YamlConfigurator
    ...
    # Extract config from +key+. If +key+ is not given or nil, the default of
    # 'log4r_config' will be used.
    def self.load_yaml_file(filename, key=nil)
      actual_load(File.open(filename), key)
    end
    ...
    def self.actual_load(yaml_docs, key=nil)
      log4r_config = nil
      key ||= 'log4r_config'

      YAML.load_documents(yaml_docs){ |doc|
        doc.has_key?(key) and log4r_config = doc[key] and break
      }

      raise ConfigError, "Key '#{key}:' not defined in yaml documents" if log4r_config.nil?
